...
// ---------------------------------------------------------------- one paper: scan and ask

/** Reads the first JSON object out of a model reply, tolerating fences and chatter. */
function jsonFrom(reply) {
  const raw = String(reply || '');
  const start = raw.indexOf('{');
  const end = raw.lastIndexOf('}');
  if (start < 0 || end <= start) throw Object.assign(new Error('The model did not answer in the expected form. Try again.'), { status: 502 });
  try {
...
/**
 * A paper as the model reads it: section headings, and a page marker on
 * every paragraph so answers can cite pages. `pageLabel` turns a PDF page
 * into the page a citation should use.
 */
export function paperText(ft, pageLabel = (p) => p, max = 150000) {
  const out = [];
  let size = 0;
  for (const s of ft.sections || []) {
    ...
    for (const p of s.paragraphs) {
      const line = `[p. ${pageLabel(p.page)}] ${p.text}\n`;
      if (size + line.length > max) return `${out.join('')}\n[The rest of the paper was cut to fit.]`;
      out.push(line);
      size += line.length;
    }
  }
...
export const SCAN_SYSTEM = `You read one scholarly paper for a researcher and report what it is, what it found, and where to look next. Work only from the text given. Page markers like [p. 12] show where each paragraph sits; cite them.

Reply with one JSON object and nothing else, no markdown fences:
{
  "summary": "3 or 4 plain sentences: what the paper asked, how, and what it found",
  "question": "the paper's research question or aim, one sentence",
  "design": "study design, or null",
  "sample": "who or what was studied and how many, or null",
  "keyFindings": ["up to 4 findings, each one sentence ending with its page, like (p. 5)"],
  "limitations": ["up to 3 limitations the authors state, each ending with its page"],
  "topics": ["4 to 6 short topic phrases, specific enough to search for"],
  "searches": ["4 searches to find related research: the same question elsewhere, the same method, work that could contradict it, and newer follow-ups"]
}
Never invent numbers, findings or pages. If the text does not say, use null or leave the list short.`;

export function buildScanMessage({ title = '', text = '' }) {
  return `Paper: ${String(title).slice(0, 400) || '(untitled)'}\n\n${String(text).slice(0, 150000)}`;
}

const list = (v, n, max = 300) => (Array.isArray(v) ? v : []).map((x) => str(x, max)).filter(Boolean).slice(0, n);

export function parseScan(reply) {
  const d = jsonFrom(reply);
  return {
    summary: str(d.summary, 1500),
    question: str(d.question, 400),
    design: str(d.design, 120),
    sample: str(d.sample, 200),
    keyFindings: list(d.keyFindings, 4),
    limitations: list(d.limitations, 3),
    topics: list(d.topics, 6, 80),
    searches: list(d.searches, 4, 160),
  };
}

export const ASK_SYSTEM = `You answer a researcher's questions about one scholarly paper, using only the text of that paper given below. Page markers like [p. 12] show where each paragraph sits.

- Answer from the paper alone. If it does not say, answer "The paper does not say" and, if useful, what it does say that is closest. Never fill gaps from general knowledge.
- Cite the page after each claim, like (p. 12). Quote short exact phrases in quotation marks when the wording matters, such as a definition or a key result.
- Keep what the authors showed apart from what they speculate or cite from others.
- Give numbers exactly as the paper states them, with their units and statistics.
- Be concise: a short paragraph or a few bullet points. Plain academic English.
- If only the abstract is available, say that your answer rests on the abstract alone.`;

/** The user turn for a question: the paper, then the conversation so far, then the question. */
export function buildAskMessage({ paper = {}, text = '', history = [], question = '' }) {
  const who = (paper.authors || []).slice(0, 4).map((a) => a.family || a.name).filter(Boolean).join(', ');
  const head = `Paper: ${paper.title || '(untitled)'}${who ? ` by ${who}` : ''}${paper.year ? ` (${paper.year})` : ''}.`;
  const past = (Array.isArray(history) ? history : []).slice(-6)
    .map((h) => `Q: ${String(h.q || '').slice(0, 1000)}\nA: ${String(h.a || '').slice(0, 3000)}`).join('\n\n');
  return [
    head,
    `<paper>\n${String(text).slice(0, 150000)}\n</paper>`,
