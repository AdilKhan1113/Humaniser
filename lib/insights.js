...
export const SYNTHESIS_SYSTEM = `You help a researcher understand what the literature says. You are given their question and a numbered set of paper abstracts. Work only from those abstracts.
...
- Do not use outside knowledge, do not invent findings, sample sizes or designs. If an abstract does not say, write null.
- Weigh the evidence: say when findings conflict, when evidence is thin, observational only, or from a narrow population. Stronger designs (meta-analyses, randomised trials) count for more.
- If the abstracts do not answer the question, say so plainly in the answer.
...
Reply with one JSON object and nothing else, no markdown fences:
{
  ...
  "papers": [
    { "n": 1, "stance": "yes" | "possibly" | "no" | "unclear", "finding": "the paper's main finding relevant to the question, at most 25 words", "design": "study design or null", "population": "who or what was studied, or null", "sample": "sample size as stated, or null" }
  ]
}`;

/** The user turn: the question, then the numbered abstracts. */
export function buildSynthesisMessage(question, works) {
  const papers = works.slice(0, MAX_SYNTHESIS_PAPERS).map((w, i) => {
    ...
    const abstract = String(w.abstract || '(no abstract available)').slice(0, 1800);
    return `[${i + 1}] ${who} (${w.year || 'n.d.'}). ${w.title}${w.venue ? `. ${w.venue}` : ''}.\n${abstract}`;
  });
...
export function parseSynthesis(reply, works) {
  ...
  const answer = (str(data.answer, 3000) || 'No answer was returned.')
