// The Anthropic provider. One of two; lib/provider.js picks between them.
//
// The offline engine handles the mechanical rules; a model handles the ones
// that need judgement, such as rhythm, metaphor and knowing when a sentence
...
export const MODEL = process.env.ANTHROPIC_MODEL || process.env.HUMANISER_MODEL || 'claude-opus-5';
// Anthropic calls this thinking depth; the UI labels the control per provider.
export const EFFORT_LABEL = 'Thinking';
...
export function buildRequest(options) {
  ...
  return {
    ...
    system: [{ type: 'text', text: options.system || HOUSE_STYLE, cache_control: { type: 'ephemeral' } }],
    messages: [{ role: 'user', content: options.userMessage || buildUserMessage(options) }],
  };
}
...
function describeError(error, Anthropic) {
  ...
  // Not a typed API error and no key in the environment: the SDK gave up before
  // it ever reached the network. Its own wording is about apiKey and auth
  // headers, which is not a useful thing to read, so say the plain version.
  if (!hasKey()) {
    const err = new Error(
      'Claude mode needs an API key, and there is none set. Stay on the offline engine, '
      + 'or add ANTHROPIC_API_KEY to a .env file and restart.',
    );
