...
//
// The model is Flash-Lite, through its `-latest` alias. It is the fastest of the
// Flash family and the least often overloaded, which matters more here than
// the last few points of quality. An alias cannot go stale the way a pinned
// version number does. Ask listModels() for what a given key can reach.
...
/**
 * Backup models, tried in order when the main one stays busy. None by
 * default: every request goes to MODEL and nothing else. List some in
 * GEMINI_FALLBACK_MODELS to opt in; each has its own free-tier allowance.
 */
 ...
 * Busy and rate-limited answers are retried, then passed to any backup models.
 */
export async function* rewrite(options, signal) {
  if (!hasKey()) throw noKey();
  // Always the configured model. A request cannot ask for a different one.
  const primary = MODEL;
  ...
  if (error?.status === 429) {
    message = error.dailyQuota
      ? `Your Gemini key has used up its free daily allowance on ${list}. It resets each day (midnight Pacific time). `
        + 'To keep going now, turn on billing for the key in Google AI Studio, or list backup models in GEMINI_FALLBACK_MODELS in .env.'
