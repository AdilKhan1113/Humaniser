...
import { toQuery } from './keywords.js';
...
const CONTACT = process.env.SCHOLAR_EMAIL || process.env.OPENALEX_EMAIL || '';
...
const SELECT = [
  ...
  'referenced_works_count', 'keywords', 'primary_topic', 'language',
].join(',');
...
async function getJson(url, signal) {
  ...
  try {
    res = await fetchImpl(url.toString(), {
      signal: combined,
      headers: { accept: 'application/json', 'user-agent': `Humaniser-Research/1.0${CONTACT ? ` (mailto:${CONTACT})` : ''}` },
    });
  } catch (cause) {
