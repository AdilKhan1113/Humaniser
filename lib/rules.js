...
import {
  AI_TELLS, INFLATED, NOMINALIZATIONS, HEDGES, EXPANSIONS,
} from './lexicon.js';
...
function flipClauses(sentence) {
  ...
  const rebuilt = parts.map((part, index) => {
    ...
    changed = true;
    return part.replace(clause, flipped);
  });
...
function trimPadding(text, log) {
  const group = HEDGES.join('|');
  let out = text;
  // "Clearly, the plan works" -> "The plan works"
  out = applyRule(out, new RegExp(`\\b(?:${group}),\\s+`, 'gi'), '', log, 'cut padding');
  // "a very good idea" -> "a good idea"
  out = applyRule(out, new RegExp(`\\b(?:${group})\\s+(?=[a-z])`, 'gi'), '', log, 'cut padding');
  return out;
}
...
function tidy(text) {
  ...
  // Sentence-initial capitals, which phrase removal often knocks out.
...
export function humanise(input, options = {}) {
  ...
  if (profile.steps.passive) text = activeVoice(text, log);
  ...
  if (profile.steps.contractions) text = addContractions(text, log, profile.contractionRatio);
