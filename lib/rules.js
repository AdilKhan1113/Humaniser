...
export const PROFILES = {
  light: {
    ...
    steps: { tells: true, inflated: true, nominals: true, splices: true, formulas: true },
    contractionRatio: 0,
    splitAt: Infinity,
  },
  balanced: {
    label: 'Balanced',
    description: 'The full house style: active voice, varied sentences, contractions, no padding.',
    steps: { tells: true, inflated: true, nominals: true, splices: true, passive: true, contractions: true, hedges: true, split: true, formulas: true, dashes: true },
    contractionRatio: 2 / 3,
    // Long, flowing sentences are welcome now; only real run-ons are split.
    splitAt: 30,
  },
  bold: {
    label: 'Bold',
    description: 'Same rules, pushed harder. More short sentences, contractions wherever they fit.',
    steps: { tells: true, inflated: true, nominals: true, splices: true, passive: true, contractions: true, hedges: true, split: true, formulas: true, dashes: true },
    contractionRatio: 1,
    splitAt: 24,
  },
...
function applyRule(text, find, use, log, rule, note) {
  ...
  return text.replace(re, (...args) => {
    const whole = args[0];
    const replacement = matchCase(whole, typeof use === 'function' ? use(...args) : use);
...
// "not only fast but also cheap" is a symmetrical formula machines love.
// "fast and cheap" says the same. Only the plain adjacent form is rewritten;
// "not only did it rain, but it also…" needs a person, and is flagged instead.
const NOT_ONLY_RE = /\bnot only\s+([^,.;:!?\n]{1,60}?),?\s+but also\s+/gi;

function unpairFormulas(text, log) {
  return text.replace(NOT_ONLY_RE, (whole, first) => {
    if (/^(?:do|does|did|is|are|was|were|has|have|had|can|could|will|would|should)\b/i.test(first)) return whole;
    const out = `${first} and `;
    log.add('no formulas', whole, out, '"not only X but also Y"');
    return out;
  });
}

// One em dash in a paragraph can earn its place; a string of them is a tell.
// A sentence with a pair of dashes gets a pair of commas instead, so the
// aside stays bracketed. A lone dash survives only once per paragraph.
const DASH_RE = /\s*(?:—|--)\s*/g;
const isRange = (full, offset, whole) => /\d$/.test(full.slice(0, offset)) && /^\d/.test(full.slice(offset + whole.length));

function calmDashes(text, log) {
  return text.split(/(\n\s*\n)/).map((para) => {
    let allowance = 1;
    return para.split(/(?<=[.!?])(\s+)/).map((sentence) => {
      const dashes = [...sentence.matchAll(DASH_RE)].filter((m) => !isRange(sentence, m.index, m[0]));
      if (!dashes.length) return sentence;
      if (dashes.length === 1 && allowance > 0) { allowance -= 1; return sentence; }
      // A bracketing pair: commas, or brackets when the aside is itself a list,
      // so "habits — hours, mornings, commutes — that" stays readable.
      if (dashes.length === 2) {
        const [a, b] = dashes;
        const inner = sentence.slice(a.index + a[0].length, b.index);
        const tail = sentence.slice(b.index + b[0].length);
        const closing = /^[.,;:!?]/.test(tail) ? '' : ' ';
        // Brackets when the aside is a list or a clause of its own ("it
        // matters"), where commas would read as a splice; commas otherwise.
        const bracket = inner.includes(',') || looksIndependent(inner);
        const swapped = bracket ? ` (${inner})${closing}` : `, ${inner}${closing ? ', ' : ''}`;
        log.add('fewer dashes', sentence.slice(a.index, b.index + b[0].length), swapped.trim(), 'a pair of em dashes');
        return sentence.slice(0, a.index) + swapped + tail;
      }
      return sentence.replace(DASH_RE, (whole, offset, full) => {
        if (isRange(full, offset, whole)) return whole;
        log.add('fewer dashes', whole, ', ', 'more than one em dash in a paragraph');
        return ', ';
      });
    }).join('');
  }).join('');
 ...
 */
/**
 * A clean-up pass for text a model has already rewritten. Only the rules that
 * are mechanical and certain run here: banned words and stock phrases, the
 * "not only... but also" formula, strings of em dashes, inflated words, and,
 * under a guide that bans them, contractions. Rhythm and voice are left to
 * the model, which has just done that work.
 */
export function polishModelOutput(input, options = {}) {
  const constraints = options.constraints || {};
  const log = new ChangeLog();
  const masked = mask(String(input || ''));
  let text = masked.text;
  text = stripAiTells(text, log);
  text = unpairFormulas(text, log);
  text = calmDashes(text, log);
  text = simplifyWords(text, log);
  if (constraints.noContractions) text = expandContractions(text, log);
  text = tidy(text);
  return { text: unmask(text, masked.vault), changes: log.entries };
}
...
export function humanise(input, options = {}) {
  ...
  if (profile.steps.formulas) text = unpairFormulas(text, log);
  if (profile.steps.dashes) text = calmDashes(text, log);
