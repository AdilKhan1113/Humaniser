...
// A marking guide is prose. Anything claiming to inflate past this is either
// broken or a zip bomb, and reading it all in before checking the size is how a
// 40 KB upload turns into a dead server.
...
async function inflateRaw(bytes) {
  ...
  // Read in chunks and stop the moment the ceiling is crossed, rather than
  // buffering the whole stream and measuring afterwards.
  for (;;) {
    ...
    if (total > MAX_INFLATED_BYTES) {
      await reader.cancel();
      throw fail('That file unpacks to far more than a marking guide should. Refusing to read it.');
    }
    parts.push(value);
  }
