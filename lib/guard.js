...
// minutes. So Claude mode sits behind a shared access code, and every route is
...
// Per-client hourly ceilings. Claude mode is the one that costs money.
const LIMITS = {
  claude: Number(process.env.RATE_LIMIT_CLAUDE || 20),
  offline: Number(process.env.RATE_LIMIT_OFFLINE || 240),
  // Rejected access codes are counted separately. Charging them to the Claude
