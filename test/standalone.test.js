...
test('nothing in it can reach the network', () => {
  // It has to work from file:// with no connection, so every asset is inline.
  // This checks fetchable positions only. A URL inside a comment is inert, and
  // the shared app.js still carries the /api calls the server build uses, which
  // the offline bridge makes unreachable rather than deleting.
